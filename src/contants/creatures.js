export const creatures = [
    {
      id: 1,
      name: "𫚖鱼",
      type: "鳞部",
      image: "/images/𫚖鱼.png",
      description: "𫚖鱼，种群在长江口多栖息于水深10米以下。",
      position: { left: 719, top: 2918 },
      size: { width: 421, height: 421 }, 
    },
    {
      id: 2,
      name: "带鱼",
      type: "鳞部",
      image: "/images/带鱼.png",
      description: "带鱼，一般栖息于大洋水域、深海、砂泥底、河口、近海沿岸，水深在60~100公尺间，也会游至深远350公尺，甚至更深之水域，产卵时则洄游至浅海水域。",
      position: { left: 259.93, top: 5168, rotation: 13.47 }, 
      size: { width: 891, height: 446 }, 
    },
    {
      id: 3,
      name: "瑇瑁",
      type: "介部",
      image: "/images/瑇瑁.png",
      description: "瑇瑁，一般在海深18.3米以上的水域中活动，一生会在几片环境完全不同的栖息地生活。",
      position: { left: 242, top: 3633 }, 
      size: { width: 750, height: 750 }, 
    },
    {
      id: 4,
      name: "海和尚",
      type: "介部",
      image: "/images/海和尚.png",
      description: "海和尚，可以潜水到约1300米的深度，并且可以屏住呼吸长达85分钟。",
      position: { left: 760, top: 9444 },
      size: { width: 444, height: 444 }, 
    },
    {
      id: 5,
      name: "海马",
      type: "鳞部",
      image: "/images/海马.png",
      description: "海马，生殖期间群体会向较浅的海域移动，寻找较合适的生产环境，其生存水深约10~30米之间。",
      position: { left: 1137, top: 4105 },
      size: { width: 513, height: 257 }, 
    },
    {
      id: 6,
      name: "海蛇",
      type: "鳞部",
      image: "/images/海蛇.png",
      description: "海蛇，喜欢在大陆架和海岛周围的浅水中栖息，在水深超过100米的开阔海域中很少见。它们有的喜欢呆在沙底或泥底的混水中，有些却喜欢在珊瑚礁周围的清水里活动。海蛇潜水的深度不等，有的深些，有的浅些。曾有人在四五十米水深处见到过海蛇。",
      position: { left: 1243, top: 4941 },
      size: { width: 355, height: 355 }, 
    },
    {
      id: 7,
      name: "海獭",
      type: "兽部",
      image: "/images/海獭.png",
      description: "海獭，傍水而居，栖于江河、湖泊、溪流、沿海淡水区域等地带，在青藏高原可分布至海拔4000米以上的河流中。",
      position: { left: 956, top: 1988 },
      size: { width: 476, height: 343 }, 
    },
    {
      id: 8,
      name: "海铁树",
      type: "介部",
      image: "/images/海铁树.png",
      description: "海铁树，栖息于水深15~20米的硬底上，或更深的珊瑚石和岩石上。",
      position: { left: 1161, top: 3094 },
      size: { width: 489, height: 478 }, 
    },
    {
      id: 9,
      name: "海蜈蚣",
      type: "虫部",
      image: "/images/海蜈蚣.png",
      description: "海蜈蚣，多栖息于潮间带和潮下带(17～53m)软泥底。",
      position: { left: 1465, top: 2160 },
      size: { width: 212, height: 212 }, 
    },
    {
      id: 10,
      name: "鲎",
      type: "介部",
      image: "/images/鲎.png",
      description: "鲎，生活在水深40米到潮间带之间的沙质海底。",
      position: { left: 930, top: 4337 },
      size: { width: 767, height: 767 }, 
    },
    {
      id: 11,
      name: "海鰌",
      type: "兽部",
      image: "/images/井鱼、海鰌、跨鲨-1.png",
      description: "海鰌，偏爱近海浅水区域，常靠近港口缓缓游动并在水面徘徊。",
      position: { left: 328, top: 2248 },
      size: { width: 969, height: 318 }, 
    },

    {
      "id": 12,
      "name": {
        "zh": "跨鲨",
        "en": "Kua Sha"
      },
      "scientificName": {
        "zh": "座头鲸",
        "en": "Humpback Whale"
      },
      "type": {
        "zh": "兽部",
        "en": "Mammalia"
      },
      "species": {
        "zh": "鲸目须 | 鲸科座 | 头鲸属 | 哺乳动物",
        "en": "Cetacea | Balaenopteridae | Megaptera | Mammal"
      },
      "distribution": {
        "zh": "座头鲸在世界各大洋均有分布，尤以南极水域较多。在中国分布于黄海、东海、南海海域。",
        "en": "Humpback whales are distributed across the world's oceans, especially in Antarctic waters. In China, they are found in the Yellow Sea, East China Sea, and South China Sea."
      },
      "description": {
        "zh": "跨鲨，潜水时露出巨大尾鳍，常将体躯跃出水面，或侧身竖起一侧鳍肢。",
        "en": "Kua Sha, known for its large tail flukes visible during dives, often breaches out of the water or raises a single flipper vertically."
      },
      "protectionLevel": {
        "cms": {
          "zh": "列入《保护野生动物迁徙物种公约》（CMS）附录Ⅰ。",
          "en": "Listed in Appendix I of the Convention on the Conservation of Migratory Species of Wild Animals (CMS)."
        },
        "cites": {
          "zh": "列入《濒临绝种野生动植物国际贸易公约》（CITES）附录Ⅰ。",
          "en": "Listed in Appendix I of the Convention on International Trade in Endangered Species of Wild Fauna and Flora (CITES)."
        },
        "iucn": {
          "zh": "列入《世界自然保护联盟濒危物种红色名录》（The IUCN Red List）：无危物种（LC），2018年评估。",
          "en": "Listed as Least Concern (LC) on the IUCN Red List of Threatened Species (2018 assessment)."
        },
        "china": {
          "zh": "列入《中国物种红色名录》：极危（CR）。\n列入《国家重点保护野生动物名录》：国家二级保护动物。\n列入《中国国家重点保护野生动物名录》一级。",
          "en": "Listed as Critically Endangered (CR) in the China Species Red List.\nIncluded in the National Key Protected Wild Animal List as a Class II protected species.\nListed in China's National Key Protected Wild Animal List as Class I."
        }
      },
      "endangerment": {
        "level": 2,
        "reasons": {
          "zh": ["海滩搁浅", "船只碰撞", "商业捕杀"],
          "en": ["Beach strandings", "Ship collisions", "Commercial whaling"]
        },
        "state": {
          "zh": "全球捕鲸业导致鲸鱼数量锐减，\n每一代人总认为自己所处的世界才是正常的世界。",
          "en": "Global whaling has drastically reduced whale populations. Each generation assumes the world they inhabit is the normal one."
        }
      },
      "image": "/images/井鱼、海鰌、跨鲨-2.png",
      "actualImage": "/actual-images/井鱼、海鰌、跨鲨-2.png",
      "position": { "left": 260, "top": 1470 },
      "size": { "width": 1095, "height": 426 },
    },

    
    {
      id: 13,
      name: "锯鲨",
      type: "鳞部",
      image: "/images/锯鲨.png",
      description: "锯鲨，一般栖息于世界热带及亚热带浅水区，有些也生活在近岸海区和江、河、湖泊的河口，栖息于水深不超过10米的浅水区域，有些进入江、河、湖泊等淡水环境中生活。甚至有些物种的锯鲨完全栖息在河口或河流上游。",
      position: { left: 282, top: 2698 },
      size: { width: 594, height: 294 }, 
    },
    {
      id: 14,
      name: "鲈鳗",
      type: "鳞部",
      image: "/images/鲈鳗.png",
      description: "鲈鳗，产卵在水域中层，水深为300～500米，盐度35以上。",
      position: { left: 716, top: 8082 },
      size: { width: 578, height: 289 }, 
    },
    {
      id: 15,
      name: "牡蛎",
      type: "介部",
      image: "/images/牡蛎.png",
      description: "牡蛎，从半咸水到海水，从潮间带到低潮线以下10多米深的海底均有牡蛎的踪迹。",
      position: { left: 1349, top: 1967 },
      size: { width: 147, height: 147 }, 
    },
    {
      id: 16,
      name: "泥翅",
      type: "虫部",
      image: "/images/泥翅.png",
      description: "泥翅，见于由潮间带到水深6100米的海域。",
      position: { left: 260, top: 2134 },
      size: { width: 179, height: 179 }, 
    },
    {
      id: 17,
      name: "泥钉",
      type: "虫部",
      image: "/images/泥钉.png",
      description: "泥钉，多栖息在高潮区和潮上带盐碱性草类丛生的泥砂中。",
      position: { left: 668, top: 1933 },
      size: { width: 198, height: 198 }, 
    },
    {
      id: 18,
      name: "七里香",
      type: "鳞部",
      image: "/images/七里香.png",
      description: "七里香，对环境条件的要求较严格，水温需在12~23℃之间，水深不得超过50米，但其活动范围常常在水下的8~12米处。",
      position: { left: 1135, top: 2614 },
      size: { width: 440, height: 440 }, 
    },
    {
      id: 19,
      name: "人鱼",
      type: "鳞部",
      image: "/images/人鱼.png",
      description: "人鱼，饱食后通常潜入30~40米的海底，仅偶尔出水换气。",
      position: { left: 801, top: 4632 },
      size: { width: 408, height: 408 }, 
    },
    {
      id: 20,
      name: "石首鱼",
      type: "鳞部",
      image: "/images/石首鱼.png",
      description: "石首鱼，栖息于水深60米以内的软泥或泥沙底质海域。",
      position: { left: 345, top: 4941 },
      size: { width: 396, height: 198 }, 
    },
    {
      id: 21,
      name: "鲥鱼",
      type: "鳞部",
      image: "/images/鲥鱼.png",
      description: "鲥鱼，一般栖息在水深30~200米的岩礁地带。",
      position: { left: 796, top: 7016 },
      size: { width: 372, height: 372 }, 
    },
    {
      id: 22,
      name: "云头鲨",
      type: "鳞部",
      image: "/images/双髻鲨、云头鲨、黄昏鲨-1.png",
      description: "云头鲨，通常被发现于近岸海洋水域以及近海深度80米（ 230英尺），包括大陆架和岛屿梯田。",
      position: { left: 1000, top: 5346.55, rotation: 73.28 },
      size: { width: 839.89, height: 419.95 }, 
    },
    {
      id: 23,
      name: "黄昏鲨",
      type: "鳞部",
      image: "/images/双髻鲨、云头鲨、黄昏鲨-2.png",
      description: "黄昏鲨，水深0~275米。",
      position: { left: 695, top: 6012 },
      size: { width: 619, height: 619 }, 
    },
    {
      id: 24,
      name: "双髻鲨",
      type: "鳞部",
      image: "/images/双髻鲨、云头鲨、黄昏鲨.png",
      description: "双髻鲨，一般在浅于20米深处出没，但最深可达到200米。",
      position: { left: 495, top: 3410.3, rotation: -22.5 },
      size: { width: 675, height: 675 }, 
    },
    {
      id: 25,
      name: "水沫鱼",
      type: "鳞部",
      image: "/images/水沫鱼.png",
      description: "水沫鱼，在大洋随洋流长距离漂游。",
      position: { left: 1432, top: 2502 },
      size: { width: 245, height: 245 }, 
    },
    {
      id: 26,
      name: "四腮鲈",
      type: "鳞部",
      image: "/images/四腮鲈.png",
      description: "四腮鲈，栖息于浅海、河口和淡水的江河湖泊等多种水域（水深0~25米）。",
      position: { left: 398, top: 3444 },
      size: { width: 289, height: 289 }, 
    },
    {
      id: 27,
      name: "土鳖",
      type: "虫部",
      image: "/images/土鳖.png",
      description: "土鳖，常栖息于河口性沿岸带的岩石、泥滩、芦苇丛和红树林的沼泽地中。",
      position: { left: 1503, top: 1209 },
      size: { width: 157, height: 157 }, 
    },
    {
      id: 28,
      name: "腽肭脐",
      type: "兽部",
      image: "/images/腽肭脐.png",
      description: "腽肭脐，栖息的水深主要在20~40米之间。",
      position: { left: 504.47, top: 4293, rotation: 17.63 },
      size: { width: 593, height: 302 }, 
    }
  ];
  